...
- No default login. First admin account must be created via `idv user create` after install. [Production install](03-install-production.md#7-create-the-first-user)
...
- `baseUrl` must equal the externally reachable address, or QR/mobile capture silently fails. [Production install](03-install-production.md#4-configure-the-address-and-https)
...
- Session data is kept forever unless retention (`cleanSessions`) is explicitly configured. [Configuration](05-configuration.md#scheduled-clean-up-jobs)
